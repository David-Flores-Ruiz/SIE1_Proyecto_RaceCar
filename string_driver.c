...
#define STRING_DISTANCE &string_distance[0] // Distancia de carrito - proyecto
...
void Float_to_String (My_float_t flotante){
	...
	string_distance[0] = Number_to_Char(g_decenas);
	string_distance[1] = Number_to_Char(g_unidades);

	string_distance[3] = Number_to_Char(g_decimas);
	string_distance[4] = Number_to_Char(g_centesimas);
}
...
uint8_t* Get_String_Distance(void) {
	uint8_t* ptr_modified_distance = STRING_DISTANCE;
	return (ptr_modified_distance);
}
